import { a as createComponent, b as createAstro, r as renderComponent, j as renderScript, d as renderTemplate, e as addAttribute, k as renderHead, g as renderSlot } from './astro/server_BEF4PhYa.mjs';
import 'kleur/colors';
/* empty css                          */
import { t as toRoutingStrategy, g as getLocaleAbsoluteUrl, a as getLocaleRelativeUrl } from './utils_BLZgBKvd.mjs';

const $$Astro$1 = createAstro();
const $$Index = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro$1, $$props, $$slots);
  Astro2.self = $$Index;
  const propsStr = JSON.stringify(Astro2.props);
  const paramsStr = JSON.stringify(Astro2.params);
  return renderTemplate`${renderComponent($$result, "vercel-analytics", "vercel-analytics", { "data-props": propsStr, "data-params": paramsStr, "data-pathname": Astro2.url.pathname })} ${renderScript($$result, "/Users/cuevaio/projects/cs-web/node_modules/@vercel/analytics/dist/astro/index.astro?astro&type=script&index=0&lang.ts")}`;
}, "/Users/cuevaio/projects/cs-web/node_modules/@vercel/analytics/dist/astro/index.astro", void 0);

var define_ASTRO_INTERNAL_I18N_CONFIG_default = { format: "directory", trailingSlash: "ignore", i18n: { defaultLocale: "en", locales: ["en", "es", "pt-br"], routing: { } }, isBuild: true };
const { trailingSlash, format, site, i18n, isBuild } = (
  // @ts-expect-error
  define_ASTRO_INTERNAL_I18N_CONFIG_default
);
const { defaultLocale, locales, domains, routing } = i18n;
const base = "/";
let strategy = toRoutingStrategy(routing, domains);
const getRelativeLocaleUrl = (locale, path, options) => getLocaleRelativeUrl({
  locale,
  path,
  base,
  trailingSlash,
  format,
  defaultLocale,
  locales,
  strategy,
  ...options
});
const getAbsoluteLocaleUrl = (locale, path, options) => getLocaleAbsoluteUrl({
  locale,
  path,
  base,
  trailingSlash,
  format,
  site,
  defaultLocale,
  locales,
  strategy,
  domains,
  isBuild,
  ...options
});

const languages = {
  en: "English",
  es: "Español",
  "pt-br": "Português"
};
const defaultLang = "en";
const ui = {
  en: {
    // Nav
    "nav.events": "Events",
    "nav.projects": "Projects",
    "nav.team": "Team",
    "nav.blog": "Blog",
    "nav.brand": "Brand",
    "nav.join": "Join",
    "nav.deck": "Deck",
    // Announcement
    "announce.text": "Code Brew events — meet shippers in LATAM.",
    "announce.cta": "See events →",
    // Hero
    "hero.headline": "The new generation of shippers in LATAM",
    "hero.sub": "A community of engineers, designers, and builders who ship products and share what they learn.",
    // What is section
    "what.heading": "What is Crafter Station?",
    "what.desc": "Crafter Station is a community born from a group of friends who wanted to build things in public — and let the world know.",
    "what.b1.title": "Product development",
    "what.b1.desc": "We ship products to learn, accelerate, and inspire LATAM.",
    "what.b2.title": "Events",
    "what.b2.desc": "Code Brews, hackathons, and meetups to connect shippers.",
    "what.b3.title": "Community",
    "what.b3.desc": "Teaching and promoting the art of shipping.",
    "what.b4.title": "Diverse team",
    "what.b4.desc": "Engineers, designers, anthropologists, statisticians — all finding their way into tech.",
    "what.b5.title": "Open calendars",
    "what.b5.desc": "Book time for mentoring, career advice, or just a chat.",
    "what.b6.title": "LATAM focused",
    "what.b6.desc": "Born in Peru. Growing across Latin America.",
    "what.b7.title": "GitHub Copilot",
    "what.b7.desc": "Log in with GitHub to use your Copilot account.",
    "what.b8.title": "Any model",
    "what.b8.desc": "75+ LLM providers — including local models.",
    "what.cta": "Read docs →",
    // Stats
    "stats.heading": "The community of shippers",
    "stats.fig1": "Fig 1.",
    "stats.fig1.label": "Community members",
    "stats.fig2": "Fig 2.",
    "stats.fig2.label": "Events hosted",
    "stats.fig3": "Fig 3.",
    "stats.fig3.label": "Shipped products",
    // Story
    "story.heading": "Our story",
    "story.p1": "We were born in late 2023 — a group of friends who met through hackathons and wanted to build something. Not just build it: ship it, share it, receive feedback, iterate, and create real value for someone in the world.",
    "story.p2": "In 2025, we decided to share what we learned with the LATAM community. We started our first Code Brew — an event to find people like us. Fifteen people showed up. Designers, engineers, self-taught developers, YouTubers, people teaching on Platzi. Cool people.",
    "story.p3": "Since then, we have hosted hackathons, more Code Brews, product launches. We knew there was something here — a culture worth growing.",
    // Pillars
    "pillars.heading": "Three pillars",
    "pillars.build.title": "Build",
    "pillars.build.desc": "Ship new products to learn and accelerate the LATAM tech ecosystem.",
    "pillars.meet.title": "Meet",
    "pillars.meet.desc": "Host events to bring shippers together and promote the shipping culture.",
    "pillars.grow.title": "Grow",
    "pillars.grow.desc": "Teach the art of shipping inside a community that celebrates it.",
    // Shipping Bible
    "bible.heading": "The Shipping Bible",
    "bible.sub": "Our philosophy, distilled. A living playbook on how to ship consistently, build in public, and grow as a builder in LATAM.",
    "bible.p1": "We ship. We build in public. We learn by doing. A shipped project teaches more than a hundred planned ones.",
    "bible.p2": "The Shipping Bible covers the full cycle — from the first spark of an idea to the follow-up after launch. Six phases, one rhythm.",
    "bible.p3": "Ship > Perfect. Build in public. Time-box everything. Story > Features. Community first. LatAm represents.",
    "bible.cta": "Read the Shipping Bible →",
    // Team
    "team.heading": "The team",
    "team.sub": "A group of engineers, designers, anthropologists, statisticians, and many more — all giving their time and energy to grow a culture of shipping.",
    "team.cta": "Meet the team →",
    // Events
    "events.heading": "Events",
    "events.sub": "Code Brews, hackathons, and product launches across LATAM.",
    "events.upcoming": "Upcoming",
    "events.past": "Past events",
    "events.cta": "See all events →",
    "events.register": "Register →",
    "events.attendees": "attendees",
    // Projects
    "projects.heading": "Shipped products",
    "projects.sub": "Products we built to learn, ship, and share.",
    "projects.cta": "View all on GitHub →",
    // Blog
    "blog.heading": "From the blog",
    "blog.sub": "Stories about shipping, building in public, and growing in tech.",
    "blog.cta": "Read all articles →",
    "blog.read": "Read article →",
    // Newsletter
    "newsletter.heading": "Be the first to know about our next events",
    "newsletter.sub": "Join the community. No spam, just shipping.",
    "newsletter.placeholder": "Email address",
    "newsletter.cta": "Subscribe",
    // Footer
    "footer.copy": "© 2026 Crafter Station",
    "footer.github": "GitHub",
    "footer.discord": "Discord",
    "footer.linkedin": "LinkedIn",
    "footer.x": "X",
    "footer.instagram": "Instagram",
    "footer.whatsapp": "WhatsApp",
    "footer.luma": "Luma",
    "footer.brand": "Brand",
    "footer.privacy": "Privacy",
    "footer.terms": "Terms",
    // Team member page
    "member.projects": "Projects",
    "member.connect": "Connect",
    "member.skills": "Skills",
    "member.back": "← Back to team",
    "member.cal.heading": "Book a meeting",
    "member.cal.sub": "Schedule a mentoring session, career advice, or just a chill chat.",
    "member.cal.cta": "Open calendar →",
    // 404
    "404.heading": "Page not found",
    "404.sub": "This page does not exist.",
    "404.cta": "← Go home"
  },
  es: {
    // Nav
    "nav.events": "Eventos",
    "nav.projects": "Proyectos",
    "nav.team": "Equipo",
    "nav.blog": "Blog",
    "nav.brand": "Brand",
    "nav.join": "Únete",
    "nav.deck": "Deck",
    // Announcement
    "announce.text": "Code Brew — conoce a shippers en LATAM.",
    "announce.cta": "Ver eventos →",
    // Hero
    "hero.headline": "La nueva generación de shippers en LATAM",
    "hero.sub": "Una comunidad de ingenieros, diseñadores y builders que shippen productos y comparten lo que aprenden.",
    // What is section
    "what.heading": "¿Qué es Crafter Station?",
    "what.desc": "Crafter Station es una comunidad nacida de un grupo de amigos que quería construir cosas en público — y contarle al mundo.",
    "what.b1.title": "Desarrollo de producto",
    "what.b1.desc": "Shipeamos productos para aprender, acelerar e inspirar a LATAM.",
    "what.b2.title": "Eventos",
    "what.b2.desc": "Code Brews, hackathons y meetups para conectar shippers.",
    "what.b3.title": "Comunidad",
    "what.b3.desc": "Enseñando y promoviendo el arte de shipear.",
    "what.b4.title": "Equipo diverso",
    "what.b4.desc": "Ingenieros, diseñadores, antropólogos, estadísticos — todos encontrando su camino en tech.",
    "what.b5.title": "Calendarios abiertos",
    "what.b5.desc": "Agenda tiempo para mentoría, consejos de carrera, o simplemente charlar.",
    "what.b6.title": "Enfocados en LATAM",
    "what.b6.desc": "Nacidos en Perú. Creciendo en toda América Latina.",
    "what.b7.title": "GitHub Copilot",
    "what.b7.desc": "Inicia sesión con GitHub para usar tu cuenta de Copilot.",
    "what.b8.title": "Cualquier modelo",
    "what.b8.desc": "Más de 75 proveedores de LLM — incluyendo modelos locales.",
    "what.cta": "Leer docs →",
    // Stats
    "stats.heading": "La comunidad de shippers",
    "stats.fig1": "Fig 1.",
    "stats.fig1.label": "Miembros de la comunidad",
    "stats.fig2": "Fig 2.",
    "stats.fig2.label": "Eventos realizados",
    "stats.fig3": "Fig 3.",
    "stats.fig3.label": "Productos shipeados",
    // Story
    "story.heading": "Nuestra historia",
    "story.p1": "Nacimos a finales de 2023 — un grupo de amigos que se conoció en hackathons y quería construir algo. No solo construirlo: shipearlo, compartirlo, recibir feedback, iterar y crear valor real para alguien en el mundo.",
    "story.p2": "En 2025 decidimos compartir lo aprendido con la comunidad LATAM. Organizamos el primer Code Brew — un evento para encontrar gente como nosotros. Llegaron 15 personas. Diseñadores, ingenieros, autodidactas, YouTubers, profes de Platzi. Gente genial.",
    "story.p3": "Desde entonces hemos organizado hackathons, más Code Brews, lanzamientos de productos. Supimos que había algo aquí — una cultura que valía la pena hacer crecer.",
    // Pillars
    "pillars.heading": "Tres pilares",
    "pillars.build.title": "Construir",
    "pillars.build.desc": "Shipear nuevos productos para aprender y acelerar el ecosistema tech de LATAM.",
    "pillars.meet.title": "Conectar",
    "pillars.meet.desc": "Organizar eventos para unir a los shippers y promover la cultura de shipear.",
    "pillars.grow.title": "Crecer",
    "pillars.grow.desc": "Enseñar el arte de shipear dentro de una comunidad que lo celebra.",
    // Shipping Bible
    "bible.heading": "The Shipping Bible",
    "bible.sub": "Nuestra filosofia, destilada. Un playbook vivo sobre como shipear de forma consistente, construir en publico y crecer como builder en LATAM.",
    "bible.p1": "Shipeamos. Construimos en publico. Aprendemos haciendo. Un proyecto shipeado ensena mas que cien planificados.",
    "bible.p2": "The Shipping Bible cubre el ciclo completo — desde la primera chispa de una idea hasta el seguimiento despues del lanzamiento. Seis fases, un ritmo.",
    "bible.p3": "Ship > Perfecto. Construye en publico. Ponle tiempo limite a todo. Historia > Funciones. Comunidad primero. LATAM representa.",
    "bible.cta": "Leer The Shipping Bible →",
    // Team
    "team.heading": "El equipo",
    "team.sub": "Ingenieros, diseñadores, antropólogos, estadísticos y más — todos dando su tiempo y energía para hacer crecer la cultura de shipear.",
    "team.cta": "Conoce al equipo →",
    // Events
    "events.heading": "Eventos",
    "events.sub": "Code Brews, hackathons y lanzamientos de productos en LATAM.",
    "events.upcoming": "Próximos",
    "events.past": "Eventos pasados",
    "events.cta": "Ver todos los eventos →",
    "events.register": "Registrarse →",
    "events.attendees": "asistentes",
    // Projects
    "projects.heading": "Productos shipeados",
    "projects.sub": "Productos que construimos para aprender, shipear y compartir.",
    "projects.cta": "Ver todos en GitHub →",
    // Blog
    "blog.heading": "Del blog",
    "blog.sub": "Historias sobre shipear, construir en público y crecer en tech.",
    "blog.cta": "Leer todos los artículos →",
    "blog.read": "Leer artículo →",
    // Newsletter
    "newsletter.heading": "Sé el primero en saber sobre nuestros próximos eventos",
    "newsletter.sub": "Únete a la comunidad. Sin spam, solo shipeo.",
    "newsletter.placeholder": "Correo electrónico",
    "newsletter.cta": "Suscribirse",
    // Footer
    "footer.copy": "© 2026 Crafter Station",
    "footer.github": "GitHub",
    "footer.discord": "Discord",
    "footer.linkedin": "LinkedIn",
    "footer.x": "X",
    "footer.instagram": "Instagram",
    "footer.whatsapp": "WhatsApp",
    "footer.luma": "Luma",
    "footer.brand": "Brand",
    "footer.privacy": "Privacidad",
    "footer.terms": "Términos",
    // Team member page
    "member.projects": "Proyectos",
    "member.connect": "Conectar",
    "member.skills": "Habilidades",
    "member.back": "← Volver al equipo",
    "member.cal.heading": "Agenda una reunion",
    "member.cal.sub": "Agenda una sesion de mentoria, consejo de carrera, o simplemente un chat.",
    "member.cal.cta": "Abrir calendario →",
    // 404
    "404.heading": "Página no encontrada",
    "404.sub": "Esta página no existe.",
    "404.cta": "← Ir al inicio"
  },
  "pt-br": {
    // Nav
    "nav.events": "Eventos",
    "nav.projects": "Projetos",
    "nav.team": "Equipe",
    "nav.blog": "Blog",
    "nav.brand": "Brand",
    "nav.join": "Entrar",
    "nav.deck": "Deck",
    // Announcement
    "announce.text": "Code Brew — encontre shippers no LATAM.",
    "announce.cta": "Ver eventos →",
    // Hero
    "hero.headline": "A nova geração de shippers no LATAM",
    "hero.sub": "Uma comunidade de engenheiros, designers e builders que fazem ship de produtos e compartilham o que aprendem.",
    // What is section
    "what.heading": "O que é Crafter Station?",
    "what.desc": "Crafter Station é uma comunidade nascida de um grupo de amigos que queria construir coisas em público — e contar ao mundo.",
    "what.b1.title": "Desenvolvimento de produto",
    "what.b1.desc": "Fazemos ship de produtos para aprender, acelerar e inspirar o LATAM.",
    "what.b2.title": "Eventos",
    "what.b2.desc": "Code Brews, hackathons e meetups para conectar shippers.",
    "what.b3.title": "Comunidade",
    "what.b3.desc": "Ensinando e promovendo a arte de fazer ship.",
    "what.b4.title": "Equipe diversa",
    "what.b4.desc": "Engenheiros, designers, antropólogos, estatísticos — todos encontrando seu caminho em tech.",
    "what.b5.title": "Calendários abertos",
    "what.b5.desc": "Agende tempo para mentoria, conselhos de carreira, ou apenas conversar.",
    "what.b6.title": "Foco no LATAM",
    "what.b6.desc": "Nascidos no Peru. Crescendo em toda América Latina.",
    "what.b7.title": "GitHub Copilot",
    "what.b7.desc": "Entre com GitHub para usar sua conta do Copilot.",
    "what.b8.title": "Qualquer modelo",
    "what.b8.desc": "75+ provedores de LLM — incluindo modelos locais.",
    "what.cta": "Ler docs →",
    // Stats
    "stats.heading": "A comunidade de shippers",
    "stats.fig1": "Fig 1.",
    "stats.fig1.label": "Membros da comunidade",
    "stats.fig2": "Fig 2.",
    "stats.fig2.label": "Eventos realizados",
    "stats.fig3": "Fig 3.",
    "stats.fig3.label": "Produtos entregues",
    // Story
    "story.heading": "Nossa história",
    "story.p1": "Nascemos no final de 2023 — um grupo de amigos que se conheceu em hackathons e queria construir algo. Não apenas construir: fazer ship, compartilhar, receber feedback, iterar e criar valor real para alguém no mundo.",
    "story.p2": "Em 2025 decidimos compartilhar o que aprendemos com a comunidade LATAM. Organizamos o primeiro Code Brew — um evento para encontrar pessoas como nós. Quinze pessoas apareceram. Designers, engenheiros, autodidatas, YouTubers, professores da Platzi. Pessoas incríveis.",
    "story.p3": "Desde então organizamos hackathons, mais Code Brews, lançamentos de produtos. Sabíamos que havia algo aqui — uma cultura que valia a pena fazer crescer.",
    // Pillars
    "pillars.heading": "Três pilares",
    "pillars.build.title": "Construir",
    "pillars.build.desc": "Fazer ship de novos produtos para aprender e acelerar o ecossistema tech do LATAM.",
    "pillars.meet.title": "Conectar",
    "pillars.meet.desc": "Organizar eventos para unir shippers e promover a cultura de ship.",
    "pillars.grow.title": "Crescer",
    "pillars.grow.desc": "Ensinar a arte de fazer ship dentro de uma comunidade que a celebra.",
    // Shipping Bible
    "bible.heading": "The Shipping Bible",
    "bible.sub": "Nossa filosofia, destilada. Um playbook vivo sobre como fazer ship de forma consistente, construir em publico e crescer como builder na LATAM.",
    "bible.p1": "Fazemos ship. Construimos em publico. Aprendemos fazendo. Um projeto entregue ensina mais do que cem planejados.",
    "bible.p2": "The Shipping Bible cobre o ciclo completo — da primeira centelha de uma ideia ao acompanhamento apos o lancamento. Seis fases, um ritmo.",
    "bible.p3": "Ship > Perfeito. Construa em publico. Coloque limite de tempo em tudo. Historia > Funcionalidades. Comunidade primeiro. LATAM representa.",
    "bible.cta": "Ler The Shipping Bible →",
    // Team
    "team.heading": "A equipe",
    "team.sub": "Engenheiros, designers, antropólogos, estatísticos e mais — todos dedicando seu tempo e energia para fazer crescer a cultura de ship.",
    "team.cta": "Conheça a equipe →",
    // Events
    "events.heading": "Eventos",
    "events.sub": "Code Brews, hackathons e lançamentos de produtos no LATAM.",
    "events.upcoming": "Próximos",
    "events.past": "Eventos anteriores",
    "events.cta": "Ver todos os eventos →",
    "events.register": "Registrar →",
    "events.attendees": "participantes",
    // Projects
    "projects.heading": "Produtos entregues",
    "projects.sub": "Produtos que construímos para aprender, fazer ship e compartilhar.",
    "projects.cta": "Ver tudo no GitHub →",
    // Blog
    "blog.heading": "Do blog",
    "blog.sub": "Histórias sobre ship, construir em público e crescer em tech.",
    "blog.cta": "Ler todos os artigos →",
    "blog.read": "Ler artigo →",
    // Newsletter
    "newsletter.heading": "Seja o primeiro a saber sobre nossos próximos eventos",
    "newsletter.sub": "Entre na comunidade. Sem spam, só ship.",
    "newsletter.placeholder": "Endereço de e-mail",
    "newsletter.cta": "Inscrever",
    // Footer
    "footer.copy": "© 2026 Crafter Station",
    "footer.github": "GitHub",
    "footer.discord": "Discord",
    "footer.linkedin": "LinkedIn",
    "footer.x": "X",
    "footer.instagram": "Instagram",
    "footer.whatsapp": "WhatsApp",
    "footer.luma": "Luma",
    "footer.brand": "Brand",
    "footer.privacy": "Privacidade",
    "footer.terms": "Termos",
    // Team member page
    "member.projects": "Projetos",
    "member.connect": "Conectar",
    "member.skills": "Habilidades",
    "member.back": "← Voltar para a equipe",
    "member.cal.heading": "Agende uma reuniao",
    "member.cal.sub": "Agende uma sessao de mentoria, conselho de carreira, ou apenas uma conversa.",
    "member.cal.cta": "Abrir calendario →",
    // 404
    "404.heading": "Página não encontrada",
    "404.sub": "Esta página não existe.",
    "404.cta": "← Ir para o início"
  }
};

const $$Astro = createAstro();
const $$BaseLayout = createComponent(($$result, $$props, $$slots) => {
  const Astro2 = $$result.createAstro($$Astro, $$props, $$slots);
  Astro2.self = $$BaseLayout;
  const { title, description, lang = defaultLang, ogImage } = Astro2.props;
  const canonicalURL = new URL(Astro2.url.pathname, Astro2.site ?? "https://crafter-station.com");
  const routePath = (() => {
    const pathname = Astro2.url.pathname;
    const [, maybeLang, ...rest] = pathname.split("/");
    if (maybeLang in languages && maybeLang !== defaultLang) {
      return `/${rest.join("/")}` || "/";
    }
    return pathname;
  })();
  return renderTemplate`<html${addAttribute(lang, "lang")} class="scroll-smooth"> <head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><link rel="icon" type="image/svg+xml" href="/favicon.svg"><meta name="description"${addAttribute(description, "content")}><meta name="generator"${addAttribute(Astro2.generator, "content")}><link rel="canonical"${addAttribute(canonicalURL, "href")}><!-- hreflang alternates -->${Object.keys(languages).map((l) => renderTemplate`<link rel="alternate"${addAttribute(l, "hreflang")}${addAttribute(getAbsoluteLocaleUrl(l, routePath), "href")}>`)}<link rel="alternate" hreflang="x-default"${addAttribute(getAbsoluteLocaleUrl(defaultLang, routePath), "href")}><!-- Open Graph --><meta property="og:title"${addAttribute(title, "content")}><meta property="og:description"${addAttribute(description, "content")}><meta property="og:type" content="website"><meta property="og:url"${addAttribute(canonicalURL, "content")}>${ogImage && renderTemplate`<meta property="og:image"${addAttribute(ogImage, "content")}>`}<meta name="twitter:card" content="summary_large_image"><meta name="twitter:title"${addAttribute(title, "content")}><meta name="twitter:description"${addAttribute(description, "content")}><!-- IBM Plex Mono — the single font for everything --><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:ital,wght@0,400;0,500;0,700;1,400&display=swap" rel="stylesheet"><title>${title}</title>${renderHead()}</head> <body class="min-h-screen bg-background text-foreground antialiased"> ${renderSlot($$result, $$slots["default"])} ${renderComponent($$result, "Analytics", $$Index, {})} </body></html>`;
}, "/Users/cuevaio/projects/cs-web/src/layouts/BaseLayout.astro", void 0);

export { $$BaseLayout as $, defaultLang as d, getRelativeLocaleUrl as g, languages as l, ui as u };
